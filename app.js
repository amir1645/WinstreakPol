...
function updateLanguage() {
    ...
    if (purchaseBtn) {
        const purchaseText = purchaseBtn.querySelector('.purchase-text');
        if (purchaseText) {
            purchaseText.textContent = t.buyTickets;
        }
    }
    ...
    if (spinBtn) {
        ...
        if (spinText) {
            spinText.textContent = t.spinWheel;
        }
    }
    ...
    if (sellBtn) {
        const sellText = sellBtn.querySelector('.sell-text');
        if (sellText) {
            sellText.textContent = t.sellPTokens;
        }
    }
    ...
    if (refreshBtn) {
        ...
        if (refreshText) {
            refreshText.textContent = t.refreshBalance;
        }
    }
...
async function connectToWallet(walletType) {
    ...
    try {
        ...
        switch (walletType) {
            ...
            default:
                await connectGenericWallet(walletType);
        }
...
// MetaMask Connection - بهبود یافته
...
async function connectTrustWallet() {
    if (window.ethereum) {
        await connectMetaMask(); // Trust Wallet uses same interface as MetaMask
    } else {
...
async function connectCoinbaseWallet() {
    if (window.ethereum) {
        ...
        userAccount = accounts[0];
        await initializeAfterConnection();
        showNotification('Connected with Coinbase Wallet!', 'success');
    } else {
        showNotification('Coinbase Wallet not detected', 'error');
    }
}

// سایر توابع اتصال کیف پول
async function connectSafePal() {
    if (window.ethereum) {
        await connectMetaMask();
    } else {
        showNotification('Please open in SafePal browser', 'info');
    }
}
...
async function connectTokenPocket() {
    if (window.ethereum) {
        ...
        showNotification('Please open in TokenPocket browser', 'info');
    }
}
...
async function connectBraveWallet() {
    if (window.ethereum) {
        ...
        showNotification('Brave Wallet not detected', 'error');
    }
}
...
async function connectPhantom() {
    if (window.phantom && window.phantom.ethereum) {
        ...
        showNotification('Phantom Wallet not detected', 'error');
    }
}

async function connectGenericWallet(walletType) {
    if (window.ethereum) {
        await connectMetaMask();
    } else {
        showNotification(`${walletType} not detected`, 'error');
    }
}

// تغییر به شبکه پالیگان - بهبود یافته
...
// Initialize after successful connection - بهبود یافته
...
// بروزرسانی موجودی کاربر - بهبود یافته
...
// بروزرسانی اطلاعات ولت - بهبود یافته
...
// تنظیم event listeners - بهبود یافته
...
// مدیریت تغییر حساب در متامسک - بهبود یافته
if (window.ethereum) {
    window.ethereum.on('accountsChanged', async (accounts) => {
        ...
        if (userAccount) {
            ...
            const connectBtn = document.getElementById('connectWallet');
            if (connectBtn) {
                connectBtn.innerHTML = `<i class="fas fa-plug"></i><span class="connect-text">${t.connectWallet}</span>`;
