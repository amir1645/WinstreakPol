...
const PTokenManager = {
    ...
    async sellPTokens() {
        try {
            ...
            sellBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> در حال پردازش...';
...
const LotteryManager = {
    ...
    setupEventListeners() {
        ...
        if (minusBtn) {
            minusBtn.addEventListener('click', () => this.changeTicketCount(-1));
        }
        if (plusBtn) {
            plusBtn.addEventListener('click', () => this.changeTicketCount(1));
        }
    },
    ...
    async buyTickets() {
        try {
            ...
            if (parseFloat(userMaticBalance) < totalCost) {
                WalletManager.showNotification(LanguageManager.get('notification-insufficient-balance'), 'error');
                return;
            }
    ...
    async refreshUserTickets() {
        try {
            ...
            document.getElementById('userPool1Tickets').textContent = `${userTickets.pool1Tickets} بلیط`;
            document.getElementById('userPool2Tickets').textContent = `${userTickets.pool2Tickets} بلیط`;
            document.getElementById('userPool3Tickets').textContent = `${userTickets.pool3Tickets} بلیط`;
...
const UIManager = {
    ...
    resetUI() {
        ...
        // Reset user tickets
        document.getElementById('userPool1Tickets').textContent = '0 بلیط';
        document.getElementById('userPool2Tickets').textContent = '0 بلیط';
        document.getElementById('userPool3Tickets').textContent = '0 بلیط';
...
const AppManager = {
    ...
    async refreshAllData() {
        ...
        try {
            ...
            // Refresh P-Token data
            await PTokenManager.refreshPTokenData();
